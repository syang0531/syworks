...
public class WandItem extends Item {
        ...
        if (active.isEmpty()) {
            if (level.isClientSide) {
                player.displayClientMessage(
                        Component.translatable("message.mcextraction.wand.no_spell").withStyle(ChatFormatting.GRAY), true);
            }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        Component affinityText = affinity == null
                ? Component.translatable("tooltip.mcextraction.wand.affinity_all").withStyle(ChatFormatting.GOLD)
                : Component.literal(affinity.glyph() + " " + affinity.displayName()).withStyle(affinity.color());
        tooltip.add(Component.translatable("tooltip.mcextraction.wand.affinity").withStyle(ChatFormatting.GRAY)
                .append(affinityText));

        List<ModSpell> bound = ModDataComponents.getBoundSpells(stack);
        Optional<ModSpell> active = ModDataComponents.getActiveSpell(stack);
        if (bound.isEmpty()) {
            tooltip.add(Component.translatable("tooltip.mcextraction.wand.empty").withStyle(ChatFormatting.DARK_GRAY));
            return;
        }
        tooltip.add(Component.translatable("tooltip.mcextraction.wand.spells", bound.size(), wand.slots())
