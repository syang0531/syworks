...
Minecraft **26.2** / **NeoForge 26.2** (Java 25) 모드. 형제 모드 `syvillage`·`sydungeon`과 같은 스택이라
함께 플레이할 수 있다.

> **0.3.0에서 마법이 빠졌다.** 지팡이·주문·룬 제단·바닐라 인챈트북 제작은 별도 모드
> **SY Magic**(`symagic`)으로 옮겨갔고, 이 모드는 금속과 합금 한 축만 담당한다.
> 왜 그랬는지와 앞으로의 방향은 [docs/재설계-제안.md](docs/재설계-제안.md).

전체 기획은 [docs/기획.md](docs/기획.md), 플레이어 안내서는 [docs/안내서.md](docs/안내서.md) 참고.
...
- ✅ 순수 금속 **10종** 잉곳 (주석·아연·니켈·알루미늄·은·크롬·티타늄·코발트·텅스텐·황)
- ✅ 합금 **10종** 잉곳 + 커스텀 티어
- ✅ 합금별 **장비 풀세트** — 도구 5종(검·곡괭이·도끼·삽·괭이) + 방어구 4종(투구·흉갑·레깅스·부츠)
- ✅ **추출로 완성** — GUI(화로 재사용) + 연료/진행도 + 추출 레시피 10종 + 호퍼 자동화
- ✅ **합금로 완성** — 입력 3슬롯 커스텀 GUI + 합금 레시피 10종
- ✅ **장비 제작 레시피** — 잉곳→장비 표준 바닐라 패턴 90종(데이터젠 자동 생성). 서바이벌 전체 루프 완성.
- ✅ 크리에이티브 탭, 한/영 이름, 플레이스홀더 텍스처(아이템·방어구 레이어)
- ✅ 런타임 검증 — 데디케이티드 서버 부팅 `Loaded 1697 recipes`, 오류 0
- ⏳ **다음 작업** — 합금 로스터 재편(수평화, 네더라이트 동급 상한), 조각 경제, 유틸 아이템.
  [docs/재설계-제안.md](docs/재설계-제안.md) §5

전체 루프: `자연블록 →(추출로)→ 순수금속 →(합금로)→ 합금잉곳 →(제작대)→ 도구·방어구`

### 사용법 (게임 내)
- **추출로**: 우클릭 → 위=소스 블록(화강암·섬록암·점토 등), 아래=연료(석탄) → 오른쪽에서 금속 잉곳 회수.
...
├── world/item/                    ModMetal, ModAlloy(밸런스)
